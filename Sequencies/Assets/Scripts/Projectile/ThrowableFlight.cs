/*using UnityEngine;          // Fast perfect. grenze nach oben fehlt
...
public class ThrowableFlight : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask wallMask0 = 0;

    // TM_Wall_02 (SideView) - NEW
    [SerializeField] private LayerMask wallMaskSide = 0;
    ...
    public void Init(Vector3 targetWorldPos, float speed, float arc, Vector2 inherited)
    {
        ...
        // ============================================================
        if (wallMaskSide.value != 0)
        {
            Collider2D sideAtTarget = Physics2D.OverlapPoint(desired, wallMaskSide);
        ...
        // ============================================================
        if (wallMask0.value != 0)
        {
            Vector2 s = groundStart;
            Vector2 d = groundTarget;

            RaycastHit2D wall = Physics2D.Linecast(s, d, wallMask0);
    ...
    private void Update()
    {
        ...
        if (sideSlidingDown)
        {
            ...
            // Still inside sidewall?
            Collider2D stillInside = Physics2D.OverlapPoint(pos, wallMaskSide);
            if (stillInside == null)
            {
                // left collider => now do spiral drop segment (like a bounce)
                sideSlidingDown = false;
                sideSpiralDropPending = true;

                // Start spiral drop immediately on next frame by forcing the "arrived" logic:
                groundPos = pos;
                groundStart = pos;
                groundTarget = pos; // so we instantly trigger the arrival branch below
            }
        ...
        {
            // 1) If we arrived on sidewall face -> start sliding out of collider
    ...
    // NEW: after leaving TM_Wall_02 collider, do a proper arc-segment downwards
    private void StartSideSpiralDrop()
    {
        ...
        // optional spin kick to match bounce feeling
        ...
        // force arc up so the vertical segment still has the same "spiral" look
        arcPerp = Vector2.up;
        totalGroundDistance = Vector3.Distance(groundStart, groundTarget);

        // no bounceDir used here
        bounceDir = Vector2.zero;
    }
    ...
    private void StartBounce()
    {
        ...
        if (wallMask0.value != 0)
        {
            RaycastHit2D wall = Physics2D.Linecast(groundPos, wanted, wallMask0);
            if (wall.collider != null)
            {
    ...
    private void Land()
    {
        ...
        if (wallMask0.value != 0)
        {
            Collider2D wallCol = Physics2D.OverlapCircle(finalGround, 0.02f, wallMask0);
            if (wallCol != null)
            {
