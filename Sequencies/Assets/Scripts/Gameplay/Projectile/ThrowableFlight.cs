...
public class ThrowableFlight : MonoBehaviour
{
    ...
    [Tooltip("Prefab mit Point Light 2D (dauerhaft). Wird an jeder Noise-Position gespawnt.")]
    [SerializeField] private GameObject noiseLightStaticPrefab;

    [Tooltip("Wenn true: Projectile-Noise spawnt dauerhaftes Licht am Noise-Punkt.")]
    ...
    private void Land()
    {
        ...
        if (!shouldDespawn && spawnPickupOnLand && pickupPrefab != null)
        {
            ...
            // Anything that spawns from a thrown projectile should NOT be usable again.
            ItemPickup ip = pickup.GetComponent<ItemPickup>();
            if (ip != null)
                ip.SetUsable(false);
        }
    ...
    // Unified "Noise" helper
    // Plays sound AND triggers ghost noise at a given position (if Player allows it).
    // ALSO: can spawn a static light at the noise position.
    // ============================================================
    private void PlayNoise(string soundKey, Vector2 worldPos, bool firstHit)
    {
        // 1) Play sound (existing system)
        ...
        // 1b) Spawn static light at noise point (optional)
